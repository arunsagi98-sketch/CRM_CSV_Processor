...
def process_rows(rows, yesterday_memory, global_min_ctr=0.37, global_max_ctr=0.55, campaign_ctr_rules=None):
    ...
    for row in rows:
        ...
        rule = find_rule(row)
        _min_ctr = rule.get('min_ctr')
        _max_ctr = rule.get('max_ctr')
        min_pct = global_min_ctr if _min_ctr is None else float(_min_ctr)
        max_pct = global_max_ctr if _max_ctr is None else float(_max_ctr)
